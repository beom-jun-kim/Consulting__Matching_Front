  ...
  <div id="layoutSidenav_content">
    <main>
      <div class="container-fluid px-4">
        <div class="row">
          <div class="col-xl-12">
            <div class="card2 mb-4" style="border:none;">
              <div class="card-body">
                    ...
                    <div class="form-row">
                      ...
                      <div class="form-group col-md-12">
                        ...
                        <textarea type="text" class="form-control" id="question" placeholder="question"
                          style="height: 30rem; width: 50rem; resize:none;" v-model="question"></textarea>
                      </div>
...
export default {
  ...
  methods: {
    ...
    async getUsr() {
      try {
        const res = await UserDataService.getUsr(this.$route.params.id);
        this.writer = res.data.email.split('@', 1);
      }
      catch (e) {
    ...
    async onSubmitForm() {
      if (!this.question) {
        this.errMessage = '질문을 등록해 주세요';
      }
      else if (!this.category) {
        this.errMessage = '카테고리를 선택해 주세요';
      } else {
        if (confirm("저장하시겠습니까?")) {
          try {
            ...
            // formData.append("writer", this.writer[0]);
            ...
            await ServCenterDataService.createQnA(this.getUserId(), data);
            if (!alert("저장되었습니다.")) {
              this.category = '';
              this.question = '';
            }
