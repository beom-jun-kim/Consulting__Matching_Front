        ...
        <div class="card2 mb-4">
            ...
            <form class="form-inline ms-auto me-0 me-md-3 my-2 my-md-0 d-flex" style="float: right">
              ...
              <div class="input-group">
                <select v-model="selectedCategory" class="form-control">
                  ...
                  <option v-for="Notice in Notices" :key="Notice.id" :value="Notice.category">
                    {{ Notice.category }}
                  </option>
                </select>
              </div>
          ...
          <div class="card-body d-flex flex-column align-items-center">
            <table class="table table-striped table-bordered">
              ...
              <!-- 조회 데이터 생성 부분 -->
              <tbody>
                <tr v-for="(Notice, index) in filteredItems" :key="index">
                  <td>
                    <input type="checkbox" v-model="NoticeIds" @click="select" :value="Notice.id" />
                  </td>
                  <td>{{ Notice.category }}</td>
                  <td>
                    <router-link :to="'/admin/NoticeDetail/' + Notice.id">
                      {{ Notice.title }}
                    </router-link>
                  </td>
                  <td>관리자</td>
            ...
            <div style="margin: 1rem auto 0; width: 100%">
              ...
              <div class="d-flex justify-content-end">
                <div>
                  <router-link to="/admin/NoticeAdd" type="button" class="btn btn-primary btn-sm but">
                    추가
                  </router-link>
                  <button type="button" class="btn btn-secondary btn-sm but" @click="NoticeDelete()">
                    삭제
                  </button>
                </div>
              </div>
  ...
  computed: {
    ...
    filteredItems() {
      ...
      return this.Notices.filter((Notice) => {
        return (
          Notice.title.toLowerCase().includes(this.search.toLowerCase()) &&
          (this.selectedCategory === "" ||
            Notice.category === this.selectedCategory)
        );
      });
    },
  },
  methods: {
    ...
    retrieveNotices() {
      ...
      ServCenterDataService.getNoticeAll()
        ...
        .then((response) => {
          this.Notices = response.data; // 객체
          // createdAt 날짜 포맷 변경
          this.Notices.forEach((Notice) => {
            Notice.formattedCreatedAt = moment(Notice.created_at).format(
              "YYYY-MM-DD"
            );
          });
        })
    ...
    NoticeDelete() {
      if (confirm("Notice(들)을 삭제하시겠습니까?")) {
        for (let i = 0; i < this.NoticeIds.length; i++) {
          ServCenterDataService.deleteNotice(this.NoticeIds[i])
            .then(() => {
              if (!alert("삭제되었습니다.")) {
    ...
    selectAll() {
      ...
      if (!this.allSelected) {
        for (let Notice in this.Notices) {
          this.NoticeIds.push(this.Notices[Notice].id);
        }
      } else {
      }
    },
    ...
    select(e) {
      ...
      if (e.target.checked == false) {
        for (let i = 0; i < this.NoticeIds.length; i++) {
          if (this.NoticeIds[i] == e.target.value) {
            this.NoticeIds.splice(i, 1);
          }
        }
      } else {
        this.NoticeIds.push(e.target.value);
      }
    },
  },
